...
SELECT category, COUNT(stock) AS total_stock FROM products GROUP BY category ORDER BY total_stock DESC;
-- result:

-- 11. Find the most expensive product price in each category.
--     Name the column max_price. Order by category alphabetically.
SELECT category, MAX(price) AS max_price FROM products GROUP BY category ORDER BY category ASC;
-- result:
